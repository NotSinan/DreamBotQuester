...
public class Main extends AbstractScript implements PaintInfo, ChatListener {
  ...
  private final CustomPaint CUSTOM_PAINT =
      new CustomPaint(
          this,
          ...
          3,
          0);
  private final RenderingHints aa =
    ...
    if (API.questVarPlayer > 0) {
      paintInfo.add(
          "Quest VarPlayer ("
              + API.questVarPlayer
              + ") value: "
              + PlayerSettings.getConfig(API.questVarPlayer));
    }
    ...
    if (API.questVarBit > 0) {
      paintInfo.add(
          "Quest VarBit ("
              + API.questVarBit
              + ") value: "
              + PlayerSettings.getBitValue(API.questVarBit));
    }
