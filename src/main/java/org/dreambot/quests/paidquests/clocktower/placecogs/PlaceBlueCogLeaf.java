...
public class PlaceBlueCogLeaf extends Leaf {
  ...
  public int onLoop() {
    ...
    if (CLOCKTOWER_FIRST_FLOOR.contains(Players.getLocal())) {
      GameObject blueSpindle =
          GameObjects.closest(
              g -> g.hasAction("Clock spindle") && g.getTile().equals(new Tile(2569, 3240, 1)));
      ...
      if (blueSpindle != null
        ...
        Sleep.sleepUntil(
            () -> !Inventory.contains("Blue cog"), () -> Players.getLocal().isMoving(), 3000, 100);
      }
    ...
    if (BLUE_COG_ROOM.contains(Players.getLocal())) {
      return GameObjectHelper.goAndInteractWithGameObject(
          BLUE_COG_ROOM, "Ladder", "Climb-up", () -> BLUE_COG_ROOM.contains(Players.getLocal()));
    }
    ...
    if (BLUE_COG_PRIOR_ROOM.contains(Players.getLocal())) {
      ...
      if (wall != null && wall.exists() && Interaction.delayEntityInteract(wall, "Push")) {
        Sleep.sleepUntil(
            () -> !BLUE_COG_PRIOR_ROOM.contains(Players.getLocal()),
    ...
    if (new Area(2623, 9599, 2558, 9664)
        .contains(Players.getLocal())) { // entire clocktower dungeon
