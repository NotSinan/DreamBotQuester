...
public class PlaceRedCogLeaf extends Leaf {
  ...
  public int onLoop() {
    ...
    GameObject redSpindle =
        GameObjects.closest(
            g ->
                g.getName().equals("Clock spindle") && g.getTile().equals(new Tile(2568, 3243, 0)));
    Item redCog = Inventory.get("Red cog");
    if (redSpindle != null
        && redSpindle.exists()
        && redCog != null
        && redCog.isValid()
        && Interaction.delayUseItemOn(redCog, redSpindle)) {
      Sleep.sleepUntil(
          () -> !Inventory.contains("Red cog"), () -> Players.getLocal().isMoving(), 3000, 100);
    }
