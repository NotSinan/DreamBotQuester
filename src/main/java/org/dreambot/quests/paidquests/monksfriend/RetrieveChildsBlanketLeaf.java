...
public class RetrieveChildsBlanketLeaf extends Leaf {
  ...
  private final Area LADDER_AREA =
      new Area(
  ...
  public boolean isValid() {
    return PlayerSettings.getConfig(PaidQuest.MONKS_FRIEND.getConfigID()) == 10
  ...
  public int onLoop() {
    ...
    if (!shouldAddCustomWebPath) {
      ...
      //            WebFinder.getWebFinder().addCustomWebPath(new
      // CustomWebPath(PATH_TO_CHILDS_BLANKET));
    ...
    //            GameObjectHelper.goAndInteractWithGameObject(LADDER_AREA, "Ladder", "Climb-down",
    // ()-> Players.getLocal().getY() == 9000);
    //        }
