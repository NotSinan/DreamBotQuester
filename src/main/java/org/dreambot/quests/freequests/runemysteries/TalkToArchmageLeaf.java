...
public class TalkToArchmageLeaf extends Leaf {
  ...
  public boolean isValid() {
    return PlayerSettings.getConfig(FreeQuest.RUNE_MYSTERIES.getConfigID()) == 1
            && Inventory.contains("Air talisman")
        || PlayerSettings.getConfig(FreeQuest.RUNE_MYSTERIES.getConfigID()) == 2
        || PlayerSettings.getConfig(FreeQuest.RUNE_MYSTERIES.getConfigID()) == 3
            && !Inventory.contains("Research package")
        || PlayerSettings.getConfig(FreeQuest.RUNE_MYSTERIES.getConfigID()) == 5
  ...
  public int onLoop() {
    ...
    final String[] DIALOGUE_OPTIONS = {
      "I'm looking for the head wizard.", "Okay, here you are.", "Go ahead.", "Yes, certainly."
    };
