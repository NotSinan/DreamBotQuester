...
public class SpinWoolLeaf extends Leaf {
  ...
  public boolean isValid() {
    ...
    return PlayerSettings.getConfig(FreeQuest.SHEEP_SHEARER.getConfigID()) == 1
        && Inventory.count(i -> i.getName().equals("Wool") || i.getName().equals("Ball of wool"))
            >= 20
  ...
  public int onLoop() {
    ...
    if (craftInterface != null && craftInterface.isVisible()) {
      if (Interaction.delayWidgetInteract(craftInterface)) {
        Sleep.sleepUntil(
            () -> !Inventory.contains("Wool"), Players.getLocal()::isAnimating, 3000, 100);
      }
      return Timing.loopReturn();
    }

    GameObject spinningWheel =
        GameObjects.closest(obj -> obj != null && obj.getName().equals("Spinning wheel"));
    if (spinningWheel != null
        && spinningWheel.exists()
        && Interaction.delayEntityInteract(spinningWheel, "Spin")) {
      Sleep.sleepUntil(
          () -> {
