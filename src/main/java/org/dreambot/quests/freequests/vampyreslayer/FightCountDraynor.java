        ...
        if (COUNT_DRAYNOR_AREA.contains(Players.getLocal())) {
            ...
            if (!Players.getLocal().isInCombat()) {
                ...
                if (coffin != null && Interaction.delayEntityInteract(coffin, "Open") &&
                        Sleep.sleepUntil(() -> Players.getLocal().isInCombat(), () -> Players.getLocal().isMoving(), 8000, 100)) {
                    return Timing.loopReturn();
                }
                NPC countDraynor = NPCs.closest(vamp ->
                        vamp.getName().equals("Count Draynor") &&
                        (vamp.getInteractingCharacter() == null || vamp.getInteractingCharacter().equals(Players.getLocal()))); //don't get any vampyres attacking other ppl
