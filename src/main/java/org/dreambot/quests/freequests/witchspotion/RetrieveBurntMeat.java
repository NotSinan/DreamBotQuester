...
public class RetrieveBurntMeat extends Leaf {
  ...
  @Override
  public int onLoop() {
    final Area PORT_SARIM_FOOD_SHOP_AREA =
        new Area(
    ...
    if (!Inventory.contains("Raw beef", "Cooked meat")) {
      ...
      if (!Shop.isOpen()) {
        NPC wydin =
            NPCs.closest(
                n ->
                    n.hasAction("Trade")
                        && n.getName().equals("Wydin")
                        && PORT_SARIM_FOOD_SHOP_AREA.contains(n));
      ...
      if (rawBeef != null
          && rawBeef.isValid()
          && rawBeef.getAmount() > 0
          && Interaction.delayItemInteract(rawBeef, "Buy 1")) {
    ...
    if (range != null && range.distance() < 10 && Interaction.delayEntityInteract(range, "Cook")) {
      Sleep.sleepUntil(
          () -> Inventory.contains("Cooked meat", "Burnt meat"),
