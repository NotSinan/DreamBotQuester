...
public class RetrieveSecondKeyLeaf extends Leaf {
  ...
  public boolean isValid() {
    return PlayerSettings.getBitValue(FreeQuest.DEMON_SLAYER.getVarBitID()) == 2
        && PlayerSettings.getBitValue(2568)
            == 1; // Player setting to determine if a player has poured water in the drain.
  }
  ...
  public int onLoop() {
    ...
    final Area VARROCK_SEWER =
        new Area(
            3224, 9899, 3232,
            9896); // This is where the key spawns after pouring water in the drain.
    return GameObjectHelper.goAndInteractWithGameObject(
        VARROCK_SEWER, "Rusty key", "Take", () -> Inventory.contains("Rusty key"));
  }
}
