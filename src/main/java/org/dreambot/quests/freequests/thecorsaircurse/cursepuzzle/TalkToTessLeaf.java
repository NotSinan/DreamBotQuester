...
public class TalkToTessLeaf extends Leaf {
  ...
  public static boolean leaveTessArea() {
    ...
    if (CHIEF_TESS_AREA.contains(Players.getLocal())) {
      GameObject vineLadder =
          GameObjects.closest(g -> g.hasAction("Climb") && g.getName().equals("Vine ladder"));
      if (vineLadder != null && vineLadder.exists()) {
        if (Interaction.delayEntityInteract(vineLadder, "Climb")) {
          Sleep.sleepUntil(
              () -> !CHIEF_TESS_AREA.contains(Players.getLocal()),
  ...
  public int onLoop() {
    ...
    return NPCHelper.goAndTalkToNpc(
        CHIEF_TESS_AREA, "Chief Tess", new String[] {"I've come to return what Arsen stole."});
  }
}
