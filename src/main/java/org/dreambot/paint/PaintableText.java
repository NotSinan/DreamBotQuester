  ...
  public void paintComponent(Graphics graphics) {
    if (infoToPaint
      ...
      graphics.fillRect(
          ...
          highestY - verticalPadding,
          (rightmostX + horizontalPadding) - (leftmostX - horizontalPadding),
          (lowestY + verticalPadding) - (highestY - verticalPadding));
      ...
      if (borderThickness
        ...
        graphics2D.setStroke(
            new BasicStroke(
                borderThickness)); // Sets graphics2D to draw a border of the specified width.
        graphics2D.drawRect(
            leftmostX - horizontalPadding,
            highestY - verticalPadding,
            (rightmostX + horizontalPadding) - (leftmostX - horizontalPadding) + borderThickness,
            (lowestY + verticalPadding) - (highestY - verticalPadding) + borderThickness);
      }
      ...
      for (int i = 0; i < infoToPaint.length; i++) {
        graphics.drawString(
            infoToPaint[i],
            xCoords[i],
            yCoords[i]); // Draws the Strings at their appropriate coordinates.
      }
    } else {
      Graphics2D graphics2D = (Graphics2D) graphics;
      Rectangle textRectangle =
          graphics
              ...
              .getStringBounds(textToPaint, graphics)
              .getBounds(); // Creates a rectangle object that is the size of the String to paint.
      int textHeight =
          (int)
              textRectangle
                  .getHeight(); // Creates textHeight variable so I can modify the rectangle.
      textRectangle.setLocation(
          xCoord - horizontalPadding + borderThickness,
          yCoord
              - verticalPadding
      ...
      textRectangle.setSize(
          ...
          (int) textRectangle.getHeight()
              + verticalPadding
              + verticalPadding); // Increases the size of the rectangle to account for padding
      ...
      if (borderThickness > 0) {
        ...
        graphics2D.setColor(borderColor);
        graphics2D.drawRect(
            xCoord - horizontalPadding,
            yCoord - verticalPadding,
            (int) textRectangle.getWidth() + borderThickness,
            (int) textRectangle.getHeight() + borderThickness);
      }
    }
  }
