...
public class CustomPaint {
  ...
  public CustomPaint(
      ...
      Color[] fontColors,
      String fontName,
      Color[] textBackgroundColors,
      Color[] textBackgroundBorderColors,
      int borderThickness,
    ...
    this.fontColors =
        copyOfNoNulls(fontColors, numberOfItems == 0 ? fontColors.length : numberOfItems);
    this.fontName =
        Arrays.stream(graphicsEnvironment.getAvailableFontFamilyNames()).anyMatch(fontName::equals)
            ? fontName
            : graphicsEnvironment
                .getAvailableFontFamilyNames()[0]; // StackExchange one liner to match a String.
    this.textBackgroundColors =
        copyOfNoNulls(
            textBackgroundColors, numberOfItems == 0 ? textBackgroundColors.length : numberOfItems);
    this.textBackgroundBorderColors =
        copyOfNoNulls(
            textBackgroundBorderColors,
            numberOfItems == 0 ? textBackgroundBorderColors.length : numberOfItems);
  ...
  public static Color makeTransparentColor(Color baseColor, int transparency) {
    return new Color(
        ...
        baseColor.getGreen(),
        baseColor.getBlue(),
        (int) (((double) ((transparency - 100) * -1) / 100) * 255));
  }
  ...
  private PaintableText createPaintableText(Graphics graphics) {
    ...
    int startingX =
        location.isXCoordinateAtRight()
            ? location.getXCoordinate() - horizontalPadding
            : location.getXCoordinate() + horizontalPadding;
    int startingY =
        location.isYCoordinateAtBottom()
            ? location.getYCoordinate()
                - verticalPadding
                - ((textHeight + spaceBetweenFields) * numberOfItems - 1)
            : location.getYCoordinate() + verticalPadding;
    for (int i = 0; i < numberOfItems; i++) {
      xCoords[i] =
          startingX
              + (xMultiplier
                  * (int)
                      fontMetrics.getStringBounds(paintInformationArray[i], graphics).getWidth());
      yCoords[i] =
          startingY
              + ((int) fontMetrics.getStringBounds(paintInformationArray[i], graphics).getHeight()
                      + spaceBetweenFields)
                  * (i + 1);
    }
    return new PaintableText(
        fontColors[0],
        textBackgroundColors[0],
        textBackgroundBorderColors[0],
  ...
  private PaintableText[] createPaintableTexts(Graphics graphics) {
    ...
    for (int i = 0; i < numberOfItems; i++) {
      ...
      yCoords[i] =
          startingY
              + (textHeight * i)
              + ((spaceBetweenFields + (verticalPadding * 2) + (borderThickness * 2)) * i);
      paintableTexts[i] =
          new PaintableText(
              fontColors[i],
              textBackgroundColors[i],
              textBackgroundBorderColors[i],
              paintInformationArray[i],
              borderThickness,
              horizontalPadding,
              verticalPadding,
              xCoords[i],
  ...
  public enum PaintLocations {
    ...
    PaintLocations(
        int xCoordinate, int yCoordinate, boolean yCoordinateAtBottom, boolean xCoordinateAtRight) {
