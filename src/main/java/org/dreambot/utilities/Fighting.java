    ...
    public static boolean eatBestFood() {
        ...
        if (haveFoodToEat()) {
            ...
            while (end.isAfter(Instant.now()) && ScriptManager.getScriptManager().isRunning() && !ScriptManager.getScriptManager().isPaused()) {
                Item bestFood = Inventory.get(i -> Arrays.stream(Food.values()).anyMatch(food -> food.foodName().equals(i.getName())));
                if (bestFood == null || !bestFood.isValid()) {
                    Logger.log("No more food found in inventory after finding some, but not ate any :-(");
                    return false;
                }
                String action = (bestFood.hasAction("Drink") ? "Drink" : (bestFood.hasAction("Eat") ? "Eat" : null));
                if (action == null) {
                    Logger.log("No action Drink or Eat found on food item: " + bestFood.getName() + ", printing options...");
