...
public class NPCHelper {
  public static int goAndInteractWithNPC(
      Area area, String npc, String action, Condition sleepUntilAfterInteract) {
  ...
  public static int goAndInteractWithNPC(
    ...
    NPC interactableNPC =
        NPCs.closest(g -> g.getName().equals(npc) && area.contains(g) && g.hasAction(action));
  ...
  public static int goAndTalkToNpc(Area area, String name, String[] dialogueOptions) {
    ...
    if (Dialogues.areOptionsAvailable()) {
      if (dialogueOptions == null || dialogueOptions.length == 0) {
        Logger.log(
            "See some options when no options passed to select in goAndTalkToNPC() function!");
        return Timing.loopReturn();
      }
      Timing.sleepForDelay();
