  ...
  public static EquipmentLoadout getBestOwnedMeleeGear() {
    ...
    for (EquipmentSlot equipSlot : EquipmentSlot.values()) {
      ...
      if (equipSlot.equals(EquipmentSlot.WEAPON)) {
        ...
        StandardMeleeWeapon bestWep =
            Arrays.stream(StandardMeleeWeapon.values())
                .filter(
                    meleeWep ->
                        attLvl >= meleeWep.getEquipmentItem().getAttLvl()
      ...
      StandardMeleeArmour bestArmorForSlot =
          Arrays.stream(StandardMeleeArmour.values())
              .filter(
                  armourPiece ->
                      defLvl >= armourPiece.getEquipmentItem().getDefLvl()
